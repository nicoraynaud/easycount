...
import { LineTemplate, LineTemplateService } from '../line-template';
...
export class LineDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.lineTemplateService.query().subscribe(
            (res: Response) => { this.linetemplates = res.json(); }, (res: Response) => this.onError(res.json()));
    }
    clear () {
    ...
    trackLineTemplateById(index: number, item: LineTemplate) {
        return item.id;
    }
